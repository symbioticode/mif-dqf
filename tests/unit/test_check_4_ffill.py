...
class TestForwardFillCheck:
    ...
    def test_sequences_truncated(self):
        """Test that sequence detection handles many sequences correctly."""
        ...
        #  FIX: Boundary condition handling
        # - Data has 20 sequences of 3 identical values (= 2 consecutive ffills)
        ...
        assert "ffill_sequences" in result.details
        ffill_seq = result.details["ffill_sequences"]

        if isinstance(ffill_seq, dict):
            # Check 'close' column was analyzed
            assert "close" in ffill_seq
            close_seq = ffill_seq["close"]

            # Verify detection: max_consecutive should be 2 (at threshold)
            max_consecutive = close_seq.get("max_consecutive", 0)
            assert max_consecutive == 2  # Exactly at threshold

            # With 20 sequences, sequence_count should be 20
            seq_count = close_seq.get("sequence_count", 0)
            assert seq_count == 20
